...
- Data is stored as objects (data plus key and metadata) inside buckets; bucket names are globally unique.
- Objects are addressed by key via the S3 API, CLI, SDKs, or console, and transferred over HTTPS.
- Data is stored redundantly across at least three Availability Zones in most storage classes.
- Buckets support features like versioning, encryption, lifecycle rules, replication, and event notifications.
- Access is controlled through [[IAM]], bucket policies, and ACLs.
...
- Backups and archival, with lifecycle policies moving data to cheaper classes over time.
- Data lakes for analytics, queried by [[Athena]], [[Glue]], and [[EMR]].
- Static website hosting and media/content distribution via [[CloudFront]].
- Application storage for images, videos, documents, logs, and user uploads.
- Big data ingestion, streaming destinations, and machine learning datasets.

### Pricing & Limits

- Billed per GB-month of storage, per request, and for data transfer out; retrieval fees apply to IA and archive classes.
- Default limit of 100 buckets per account (adjustable) and a single object size limit of 5 TB.
- Single PUT limited to 5 GB; objects over 5 GB require multipart upload.
- Storage cost decreases as classes get colder, while retrieval cost and minimum durations increase.
